...
navbarMenu( '消費額度調查(依地區)',
  ...
  tabPanel(
    ...
    sidebarLayout(
      sidebarPanel(
        selectInput('PAY4','消費類別:',
                    choices = colnames(dta4)[25:30]),
        hr(),
        helpText("資料來自聯合信用卡處理中心")
      ),
      mainPanel(plotOutput("phonePlot4"))
